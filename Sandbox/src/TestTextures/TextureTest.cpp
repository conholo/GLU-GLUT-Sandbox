...
int TextureTestLayer::CreateTexturedGeometryDrawList(Core::PrimitiveType type)
{
	Core::Geometry* geometry = Core::Geometry::Create(type);

	int cubeList = glGenLists(1);
	...
	{
		const Core::Vertex vertex = geometry->GetVertices()[index];
...
void TextureTestLayer::CreateTexture()
{
	int width, height;
	const char* filePath = "src/assets/textures/worldTex.bmp";
...
void TextureTestLayer::OnAttach()
{
	CreateTexture();
	CubeDrawList = CreateTexturedGeometryDrawList(Core::PrimitiveType::Cube);
}

void TextureTestLayer::OnDetach()
{

}

void TextureTestLayer::OnDisplay(float deltaTime)
{
	Core::OrbitalControls::UpdateViewProjection(Core::ProjectionType::Perspective, deltaTime);


	glCallList(CubeDrawList);

	glutSetWindow(Core::Window::GetMainWindow());
	glutPostRedisplay();
}

void TextureTestLayer::OnAnimate(float deltaTime)
{
	glutSetWindow(Core::Window::GetMainWindow());
	glutPostRedisplay();
}

void TextureTestLayer::OnEvent(Core::Event& event)
{
